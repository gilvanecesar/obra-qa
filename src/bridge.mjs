...
export function createBridge({token,projects,out='runs',runner=audit,tls,pdfPython=process.env.OBRA_QA_PDF_PYTHON}){
 ...
 const handler=async(req,res)=>{
  ...
  try{
   ...
   if(!value||Object.keys(value).some(k=>!['project','revision'].includes(k))||!Object.hasOwn(projects,value.project)||(value.revision!==undefined&&!/^[a-f0-9]{40,64}$/.test(value.revision)))return send(res,400,{error:'Use a configured project and full commit SHA only'});
   ...
   if(value.revision===undefined)value.revision=resolveRevision(projectConfig.repo);
   const id=randomUUID();jobs.set(id,{id,state:'running',project:value.project,revision:value.revision,events:[]});busy=true;
   send(res,202,jobs.get(id));
   const p=projects[value.project];
   ...
   }).catch(()=>jobs.set(id,{...jobs.get(id),state:'error',error:'Audit failed to start or complete; inspect operator logs'})).finally(()=>{busy=false;});
